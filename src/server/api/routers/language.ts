...
import { z } from "zod";
import type { ApiResponseProps, LanguageServiceProps } from "~/types";
...
export const languageRouter = createTRPCRouter({
  getApplicationStatus: protectedProcedure.query(async ({ ctx }) => {
    try {
      const response = await axios.get<ApiResponseProps<LanguageServiceProps>>(
        `${env.API_URL}/language/application-status/${ctx.session.user.id}`,
      );
  ...
  getAllWithPagination: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        const response = await axios.get<ApiResponseProps<LanguagePaginationProps>>(
          `${env.API_URL}/language/all?limit=${input.limit}&cursor=${input.cursor ?? 0}`,
        );
        return response.data.data;
      } catch (error) {
        if (error instanceof TRPCClientError) {
    ...
    .mutation(async ({ input }) => {
      try {
        await axios.post<ApiResponseProps<null>>(
          `${env.API_URL}/language/create-booking`,
          input,
        );
      } catch (error) {
        if (error instanceof TRPCClientError) {
