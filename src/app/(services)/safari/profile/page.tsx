...
import { api } from "~/trpc/server";
import type { SafariDetailProps } from "~/types";
import { SafariProfileTabs } from "~/components/safari/profile-tabs";

type PageProps = {
  searchParams: Promise<{ safariId: string; tab?: string }>;
};
...
export default async function SafariPage({ searchParams }: PageProps) {
  ...
  const initialTab =
    paramProps.tab === "packages" || paramProps.tab === "booking"
      ? paramProps.tab
      : "general";

  return <SafariProfileTabs safari={safari} initialTab={initialTab} />;
}