...
type EcoTransitInputProps = z.infer<typeof ecoTransitFormSchema>;
...
export const EcoTransitForm = () => {
  const { toast } = useToast();
  const [showPassword, setShowPassword] = useState<boolean>(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState<boolean>(false);
  const form = useForm<EcoTransitInputProps>({
    resolver: zodResolver(ecoTransitFormSchema),
  });

  const createEcoTransit = api.register.createEcoTransit.useMutation({
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Eco Transit profile created successfully",
      });
      form.reset();
    },
    onError: (error) => {
                  ...
                  <FormControl>
                    <div className="flex-start mt-2 flex">
                      <UploadButton
                        ...
                        onClientUploadComplete={(res) => field.onChange(res[0]?.appUrl)}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="123 Eco Street" {...field} value={field.value ?? ''} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="mt-4">
                ...
                <FormDescription>
                  Describe your transportation services and experience
                </FormDescription>
                <FormControl>
                  <Textarea
                    placeholder="Tell us about your transportation services, specialties, and achievements..."
